...
export function CodexAccountsPage() {
  ...
  const handleApiKeyWriteModeChange = useCallback(
    ...
    [savingApiKeyWriteModeId, t, updateApiKeyWriteMode],
  );

  const renderApiKeyWriteModeControl = useCallback(
    (account: CodexAccount): ReactElement => (
      <div className="account-sub-line codex-provider-inline-line">
        <span className="codex-login-subline codex-provider-inline-text">
          {t("codex.api.writeMode.label", "写入模式")}：
        </span>
        ...
        >
          ...
          {isApiKeyAccount && (
            <>
              ...
              {renderApiKeyWriteModeControl(account)}
            </>
          )}
